...
module Algs4ruby
  module Sorting
    class HeapTest < MiniTest::Unit::TestCase
      ...
      def test_sort
        assert_equal @array.sort, Heap.sort(@array)
        # assert_equal [9, 3, 5, 2, 0, 6, 4, 5, 3, 5], @array
      end

      def test_sort_by_specific_order
        assert_equal @array.sort.reverse, Heap.sort(@array){|a,b| a > b}
        # assert_equal [9, 3, 5, 2, 0, 6, 4, 5, 3, 5], @array
      end

      def test_sort_stability
        refute_equal ['Bryant', 'Dapian', 'Larry', 'Liuwei'],
          Heap.sort(@pairs){|a,b| a.number < b.number }.select{|p| p.number == 8}.map(&:name)
      end
    end
  end
end
